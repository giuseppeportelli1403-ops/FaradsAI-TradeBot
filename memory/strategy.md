...
5. **Range Sweep Reversal** *(added 2026-04-29 to handle pre-FOMC / consolidation regimes when most of the universe is neutral)*
   ...
   - **Sweep:** on the 15M, a wick exceeds the range extreme (high or low) by **≥ max(2 × current spread, 0.10 × 15M ATR)** — both floors apply. The 2× spread floor catches "real sweep vs spread tag"; the 0.10 × ATR floor catches "real sweep vs noise wick on a high-vol candle". Stricter than trigger 3's 1× spread because reversal-from-range is lower-probability than reversal-with-trend.
...
| 2026-04-29 | Manual (range-mode addition) | **5th trigger: Range Sweep Reversal.** Activates only on neutral-bias instruments. Range definition (≥ 8 1H candles, width ≥ 1.5 × ATR), sweep ≥ max(2× spread, 0.10× ATR), reversal candle within 2 candles, body ≥ 0.6× range, closes back inside range by ≥ 1× spread. TP1 = mid-range, TP2 = opposite extreme, TP3 = measured-move projection. **Half-size posture: 0.25% total risk.** Tier 3 only (capped at score 59 by scanner). Tighter R:R: TP1 1:1, TP2 1.5:1, TP3 2:1. **Cat A opposing news INVALIDATES** the setup (overrides the global trend-mode half-size rule for range-mode specifically — reversal premise breaks under news-driven continuation pressure). Required `setup_type` for proposals: `Range_Sweep_Reversal` (canonical underscore form). | Captures range-bound regimes where the 4 trend-mode triggers find nothing — observed 5/7 universe filtered as neutral during pre-FOMC 2026-04-29 |

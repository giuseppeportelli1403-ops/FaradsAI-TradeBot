...
// ==================== DB-MODE RENDERER (T040 / Spec 001) ====================
//
// 2026-05-12 — added by Migration 007 / spec/scoring-pipeline-audit.
// Reads directly from the new trade_rejections table (and the
// analyst_log.category columns) via buildDailyDigest, bypassing the
// pm2-out.log regex parser entirely. The DB path is the source of
// truth post-Migration 007; the legacy log-scrape path stays available
// via the default code path AND can be opted into explicitly via
// --legacy for one release.
//
// CLI:
//   tsx scripts/dump-reject-metrics.ts            → legacy log-scrape (default for one release)
//   tsx scripts/dump-reject-metrics.ts --db       → read from trade_rejections + analyst_log
//   tsx scripts/dump-reject-metrics.ts --legacy   → explicit legacy mode (same as default for now)
//   tsx scripts/dump-reject-metrics.ts <date>     → date arg, mode flag may follow
//   tsx scripts/dump-reject-metrics.ts --db <date>

export function renderDigestMarkdown(
  digest: { date: string; total_rejections: number; fail_closed_total: number; by_category: Record<string, number | undefined>; has_other: boolean },
  generatedAt: string,
): string {
  const sortedEntries = Object.entries(digest.by_category)
    .sort((a, b) => (b[1] ?? 0) - (a[1] ?? 0));
  const rows = sortedEntries
    .map(([cat, count]) => `| ${cat} | ${count ?? 0} |`)
    .join('\n');
  const otherWarning = digest.has_other
    ? '\n> ⚠️ `OTHER` or unknown category present — investigate (SC-002 violation).\n'
    : '';
  return `# Farad Reject Metrics — ${digest.date} (UTC, DB mode)

Generated: ${generatedAt} · Source: trade_rejections + analyst_log (Migration 007)

## Summary
- Total rejections: **${digest.total_rejections}**
- Fail-closed rejections: **${digest.fail_closed_total}** (analyst API failures, parse failures, missing tool calls)
- Cause-rejected: **${digest.total_rejections - digest.fail_closed_total}** (deliberate analyst REJECT or executor gate)
${otherWarning}
## By category (sorted by count, descending)

| Category | Count |
|---|---|
${rows || '_No rejections recorded._'}

---

_Generated by scripts/dump-reject-metrics.ts in --db mode. Legacy log-scrape mode remains the default; pass --db explicitly to switch._
`;
}
...
if (isMain) {
  // Parse CLI args: extract --db, --legacy flags and the optional YYYY-MM-DD date.
  const args = process.argv.slice(2);
  const useDb = args.includes('--db');
  // --legacy flag is currently a no-op (legacy is default) but accepted for forward
  // compatibility — once the default flips to --db, --legacy will mean something.
  ...
  if (useDb) {
    // === DB MODE — Migration 007 trade_rejections + analyst_log.category ===
    // Lazy import so test imports of this file don't drag the SQL.js WASM.
    Promise.resolve()
      .then(async () => {
        const { initDatabaseAsync } = await import('../src/database/index.js');
        const { buildDailyDigest } = await import('../src/rejection-log/digest.js');
        await initDatabaseAsync();
        const digest = buildDailyDigest(targetDate);
        writeFileSync(outputPath, renderDigestMarkdown(digest, generatedAt), 'utf-8');
        console.log(`[reject-metrics] (db mode) Wrote ${outputPath}`);
        console.log(`[reject-metrics] (db mode) ${digest.total_rejections} rejections (${digest.fail_closed_total} fail-closed)${digest.has_other ? ' — ⚠️ OTHER present' : ''}`);
      })
      .catch((err) => {
        console.error(`[reject-metrics] DB mode failed: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      });
  } else {
    // === LEGACY MODE — pm2-out.log regex scrape (default for one release) ===
    ...
    console.log(`[reject-metrics] (legacy mode) Wrote ${outputPath}`);
    const totalSkips = Object.values(report.skipsByCategory).reduce((a, b) => a + b, 0);
    console.log(`[reject-metrics] (legacy mode) ${report.totalCycles} cycles, ${report.placeOrderCount} place_orders, ${totalSkips} skips`);
  }
}
